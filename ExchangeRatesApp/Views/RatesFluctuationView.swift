...
struct RatesFluctuationView: View {
        ...
        NavigationView {
            ...
            .onChange(of: searchText) { searchText in
                if searchText.isEmpty {
                    return viewModel.searchResults = viewModel.ratesFluctuation
                } else {
                    return viewModel.searchResults = viewModel.ratesFluctuation.filter { $0.symbol.contains(searchText.uppercased()) ||
                        $0.change.formatter(decimalPlaces: 6).contains(searchText) ||
                        $0.changePct.toPercentage().contains(searchText) ||
    ...
    private var ratesFluctuationListView: some View {
        List(viewModel.searchResults) { fluctuation in
            NavigationLink(destination: RateFluctuationDetailView(baseCurrency: "BRL", fromCurrency: fluctuation.symbol)) {
                VStack {
